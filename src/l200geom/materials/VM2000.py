...
def initialize_vm2000_spectrum():
    ...
    try:
        with open(path + "/VM2000_em_spec.dat", 'r') as file: #"VM2000_em_spec.dat" TPBOnVM2000Emission
...
def VM2000EmissionSpectrum(frequencyV, efficiencyV, successfulInitialization, npoints, energy):
    ...
    if successfulInitialization:
        ...
        eff = ((targetf - frequencyV[j]) * (efficiencyV[j + 1] - efficiencyV[j]) /
                (frequencyV[j + 1] - frequencyV[j]))
        eff += efficiencyV[j]
        return eff
    else:
        # Return default efficiency if initialization failed
        print("VM2000 initialization failed.")
        return 0.2
...
def CalculateWLSmfp(yield_value):
    ...
    if yield_value < 0 or yield_value > 1:
        print("Warning: Yield cannot be larger than 100%!")
        print("Warning: Yield scaled to 100%")
    ...
    mfp = -totalPath / help_value  # Mean free path calculation --> how longth can photon propagate in foil without absorption
...
def VM2000_parameters() -> tuple[Quantity, Quantity, Quantity, Quantity, Quantity]:
    ...
    for ji in range(1, num1):
        ...
        WLS_emission[ji] = VM2000EmissionSpectrum(frequencyV, efficiencyV, successfulInitialization, npoints, VM2000_energy_range[ji])
