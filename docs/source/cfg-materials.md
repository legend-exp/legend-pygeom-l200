...
{mod}`pygeomoptics` documentation. Especially important is the functionality of
the {mod}`pygeomoptics.store` which allows users to easily swap some of the
...
import pint
from pygeomoptics.lar import lar_scintillation_params
...
{meth}`pygeomoptics.lar.lar_scintillation_params`) to 40 photons/keV.
